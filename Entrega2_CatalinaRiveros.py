...
# %%
# ── Comparison: Original image vs Preprocessed image ────────────────────
import matplotlib.gridspec as gridspec

_orig_arr = np.array(Image.open(path).convert("RGB"))
_proc_arr = skio.imread(path_proc)
if _proc_arr.ndim == 2:                    # grayscale → RGB for uniform display
...
#plt.suptitle('Original vs Preprocessed', fontsize=16, fontweight='bold', y=1.01)
...
#axes_pipe[3].imshow(~skeleton, cmap='gray')
#axes_pipe[3].set_title('Skeletonized Image', fontsize=12)
#xes_pipe[3].axis('off')
