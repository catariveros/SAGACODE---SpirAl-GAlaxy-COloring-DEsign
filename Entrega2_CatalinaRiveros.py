...
def duplicate_curve(curve, offset=12):
    dupl = curve.copy()
    n = len(curve)
    for i in range(1, n - 1):
        dx = curve[i+1, 0] - curve[i-1, 0]
...
# %%
def draw_sparkle(ax, cx, cy, size=8, color='grey', lw=0.8):
    long_arm  = size
    short_arm = size * 0.35

    lines = [
        [(cx, cy - long_arm),  (cx, cy + long_arm)],
        [(cx - long_arm, cy),  (cx + long_arm, cy)],
        [(cx - short_arm, cy - short_arm), (cx + short_arm, cy + short_arm)],
        [(cx - short_arm, cy + short_arm), (cx + short_arm, cy - short_arm)],
    ]
...
labeled_cells = label(metrics["paintable_mask"], connectivity=1)
cell_props     = regionprops(labeled_cells)
centroids      = np.array([r.centroid for r in cell_props])   

fig, axes = plt.subplots(2, 1, figsize=(5, 10), dpi=150, constrained_layout=True)

def draw_paintable_base(ax):
    overlay = np.ones((*skeleton.shape, 3), dtype=np.uint8) * 255
    overlay[skeleton > 0]              = [0, 0, 0]
    overlay[metrics["paintable_mask"]] = [235, 180, 190]
    ax.imshow(overlay)
...
ax = axes[0]
draw_paintable_base(ax)
sample = rng.choice(len(centroids), size=min(40, len(centroids)), replace=False)
for i in sample:
    cy, cx = centroids[i]
    area   = cell_props[i].area
    radius = max(1, np.sqrt(area) * 0.08)
    circle = plt.Circle((cx, cy), radius, color='blue', alpha=1)
    ax.add_patch(circle)
for ctrl, curve in results:
    ax.plot(curve[:,0], curve[:,1], lw=0.8, color='grey', alpha=0.6)
ax.set_title('Dots',fontsize=13)
...
sparkle_sample = rng.choice(len(ctrl_pts), size=min(20, len(ctrl_pts)), replace=False)
for i in sparkle_sample:
    cy, cx = ctrl_pts[i]  
    draw_sparkle(ax, cx, cy, size=7, color='blue', lw=1)

ax.set_title('Sparkles',fontsize=13)


plt.savefig('ornamentation_action_space_elements.png', bbox_inches='tight', dpi=300)
plt.show()
print('Saved: ornamentation_action_space_elements.png')
