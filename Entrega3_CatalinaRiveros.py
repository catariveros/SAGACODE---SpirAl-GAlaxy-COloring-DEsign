...
# ### 3. DexiNed execution
# 
# DexiNed is applied to the preprocess image, edited by the user.
...
plt.savefig('comparison_original_vs_preprocessed.png', bbox_inches='tight', dpi=300)
plt.show()
...
plt.savefig('comparison_dexined_binary_skeleton_spline_horizontal.png', bbox_inches='tight', dpi=300)
plt.show()
...
# ### 7. Paintable Area
# 
# In order to maximaze the paintable area in the next part of the project, the first step is to calculate the paintable area.
...
for ax in axes:
   ax.axis("off")
plt.savefig('paintable_area.png', bbox_inches='tight', dpi=500)
plt.tight_layout()
plt.show()
...
fig, axes = plt.subplots(2, 1, figsize=(5, 10), dpi=150, constrained_layout=True)
...
plt.savefig('action_space_examples_base.png', bbox_inches='tight', dpi=300)
plt.show()
...
# %%
def run_base_design_iterations(curves, shape, n_iterations=5, seed=42,max_cell_area_ref=None, pool_size=20):
    ...
    for it in range(1, n_iterations + 1):
        ...
        print(f"  Elegido: {chosen['label']}  (área: {chosen['area']:.0f} px²  "
...
results = [(None, c) for c in final_curves]   # 'results' queda actualizado con el nuevo diseño base
...
ax.axis('off')
plt.savefig('final_based_design,png')
plt.show()

# %%

def draw_diamond(ax, cx, cy, size=8, color='grey', lw=0.8):
    """Dibuja un diamante (cuadrado rotado 45°) centrado en (cx, cy)."""
    diamond = plt.Polygon(
...
def run_ornamentation_iterations(curves, shape, n_iterations=3, seed=99,
    ...
    for it in range(1, n_iterations + 1):
        ...
        chosen = candidates[chosen_idx]
        print(f"  Elegido: +{chosen['added_area']:.0f} px² adicionales")
...
ax.set_title(f"Diseño final para colorear — área total: {orn_total_mask.sum():.0f} px²")
ax.axis('off')
plt.savefig('final_design_ornamented.png', bbox_inches='tight', dpi=300)
plt.show()
...
def fill_paintable_solid(curves, shape, max_hole_area=500):

    skel_render    = rasterize_design(curves, shape)
    metrics        = evaluate_paintable_cells(skel_render)
    paintable_mask = metrics["paintable_mask"]

    union = paintable_mask | skel_render

    background     = ~union
    labeled_holes, n_holes = nd_label(background)

    hole_sizes = np.bincount(labeled_holes.ravel())
    exterior_label = hole_sizes[1:].argmax() + 1  

    filled_solid = union.copy()
    for i in range(1, n_holes + 1):
        if i == exterior_label:
            continue                        
        hole_px = int(hole_sizes[i])
        if hole_px <= max_hole_area:       
            filled_solid[labeled_holes == i] = True

    n_original = int(paintable_mask.sum())
    n_filled   = int(filled_solid.sum())

    return filled_solid, paintable_mask

filled_solid, paintable_original = fill_paintable_solid(
    final_curves, skeleton.shape, max_hole_area=500)

fig, axes = plt.subplots(1, 2, figsize=(14, 7), dpi=200)

overlay_orig = np.ones((*skeleton.shape, 3), dtype=np.uint8) * 255
overlay_orig[paintable_original] = [235, 180, 190]
axes[0].imshow(overlay_orig)
axes[0].axis('off')

overlay_fill = np.ones((*skeleton.shape, 3), dtype=np.uint8) * 255
overlay_fill[filled_solid] = [235, 180, 190]
axes[1].imshow(overlay_fill)

for cy, cx, radius in all_dots:
    axes[1].add_patch(plt.Circle((cx, cy), radius,
                                  fill=False, edgecolor='black', linewidth=1.2, zorder=10))
for cy, cx in all_diamonds:
    size = 7
    axes[1].add_patch(plt.Polygon(
        [[cx, cy-size], [cx+size, cy], [cx, cy+size], [cx-size, cy]],
        closed=True, fill=False, edgecolor='black', linewidth=1.2, zorder=10))

axes[1].axis('off')
plt.tight_layout()
plt.savefig("paso1_filled_solid.png", bbox_inches='tight', dpi=300)
plt.show()
print("Saved: paso1_filled_solid.png")

# %%
from scipy.ndimage import binary_erosion

def get_border_pixels(filled_solid):
    eroded = binary_erosion(filled_solid, iterations=1)
    border = filled_solid & ~eroded
    return border

border_pixels = get_border_pixels(filled_solid)
fig, ax = plt.subplots(figsize=(8, 8), dpi=200)

overlay = np.ones((*skeleton.shape, 3), dtype=np.uint8) * 255
overlay[filled_solid] = [235, 180, 190] 
overlay[border_pixels] = [0, 0, 0]        
...
for cy, cx in all_diamonds:
    ...
    ax.add_patch(plt.Polygon(
        [[cx, cy-size], [cx+size, cy], [cx, cy+size], [cx-size, cy]],
        closed=True, fill=False,
        edgecolor='black', linewidth=0.8, zorder=20))

ax.axis('off')
plt.title('Final design', fontsize=12)
plt.savefig("final_design.png", bbox_inches='tight', dpi=300)
plt.show()
plt.savefig("final_design.pdf", format="pdf", bbox_inches="tight")
print("Saved: paso2_border_pixels.png")
